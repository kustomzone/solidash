...
class TripledocProfile extends LitElement {
  ...
  render() {
    ...
    ${friends.map((f) => html`
      ...
      <p>Storage : <button id="storageBtn" @click=${this.clickFolder} uri=${this.profile.storage} >${this.profile.storage}</button></p>
      `;
    }
    ...
    uriChanged(uri){
      ...
      this.th.getProfileFromCard(uri).then( p =>{
        ...
        var message = {action:"exploreFolder", uri:this.profile.storage}
        console.log(message)
  ...
  clickFolder(event) {
  //  var uri = event.target.getAttribute("uri");
  var uri = this.shadowRoot.getElementById("storageBtn").getAttribute("uri");
  console.log(uri)
